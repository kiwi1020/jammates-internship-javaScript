...
def modify_filename(filename):
    # Replace keys (e.g., Ab, Bb, C#, Db) in the filename with (key)
    return re.sub(r'\b([A-G][b#]?)\b', r'(\1)', filename)
    ...
    for root, dirs, files in os.walk(input_dir):
        for filename in files:
            if filename.endswith(('.png', '.jpg', '.jpeg')):
                ...
                # Modify the filename
                modified_filename = modify_filename(os.path.splitext(filename)[0])
                output_html_path = os.path.join(output_subdir, f"{modified_filename}.html")
...
output_dir = r'C:\Users\애드업\Desktop\test\임시2'
